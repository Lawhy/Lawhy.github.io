...
For the forward propagation, the input is the one-hot encoding vector $$x$$ for the current center word, and the corresponding word vector can be obtained by $$h = W_{input}^T x$$ (with shape $$N \times 1$$). The output matrix $$W_{output}$$ are word vectors for context words, so $$W_{output} h$$ will return the similarity vector of shape $$\lvert V \rvert \times 1$$ with the $$i$$th cell storing a similarity score between the input center word and the $$i$$th word in the vocabulary.
