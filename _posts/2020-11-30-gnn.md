...
### Model
The goal of GNN is to learn a `state` embedding $$\mathbf{h}_v \in \mathbb{R}^s$$ which contains the information of *neighborhood* of each node. The state embedding $$\mathbf{h}_v$$ is an $$s$$-dimension vector of node $$v$$ and can be used to produce an output $$\mathbf{o}_v$$ such as the node label.
