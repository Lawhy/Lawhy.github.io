...
13. **Knowledge Graph Embeddings**
    ...
    13.3 <u>Tensor decomposition models</u>: the original tensor is decomposed into more *elementary* tensors which capture *latent factors* (underlying information). `Rank decomposition` such that \\(\sum_r x_1 \otimes y_1 = C \\) is approximated by setting a limit \\( d < r\\), \\(\otimes\\) refers to the `outer product` of vectors. Method is called `Canonical Polyadic` (CP) decomposition. For graphs, we can have it encoded as a one-hot, order-3 tensor \\(G\\) with \\(|V| \times |L| \times |V|\\) elements, where \\(G_{ijk}\\) is set to one if \\(i^{th}\\) node links to \\(k^{th}\\) node with an edge having \\(j^{th}\\) label, or zero otherwise. Apply CP we obtain \\( \sum^d_{i=1} x_i \otimes y_i \otimes z_i \approx G\\) and the decomposed \\(x_i, y_j, z_k\\) are the graph embeddings we want.
