...
$$
L_i = H(\mathbf{y}, \hat{\mathbf{y}}) = - \sum_{j=1}^{\lvert V \rvert} y_j \log \hat{y}_j = - \log \hat{y}_i = - \log \frac{\exp(f_{y_i})}{\sum_{j=1}^{\lvert V \rvert} \exp(f_j))}
$$

where $$y_j = 1 \iff j=i$$ because of the nature of the one-hot encoding and the output is generated by the softmax activation.
