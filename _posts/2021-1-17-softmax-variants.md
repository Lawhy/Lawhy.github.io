...
where $$P^{\mathcal{D}_c}(w)$$ is the probability of the word $$w$$ occured in the context $$c$$, and we want to fit the model $$P^+(\cdot | c)$$ to $$P^{\mathcal{D}_c}(\cdot)$$ such that $$P^+(\cdot | c) = \hat{P}^{\mathcal{D}_c}$$. Notice that in the third line, we discard the data probability because every word-context pair (regardless of repetition) occurs only once in the dataset.
