...
$$
L = H(\mathbf{y}, \hat{\mathbf{y}}) = - \sum_{j=1}^{\lvert V \rvert} y_j \log \hat{y}_j = - \log \hat{y}_i = - \log \frac{\exp(z_{i})}{\sum_{j=1}^{\lvert V \rvert} \exp(z_j))}
$$

where $$y_j = 1 \iff j=i$$ because of the nature of the one-hot encoding, $$z$$ refers to the result generated from the previous layer, and the output is computed by the softmax activation.
