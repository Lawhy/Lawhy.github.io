...
$$
\mathbb{E}_{Uniform(0, n)} =\mathbb{E}_{Q}[\exp(z) \frac{P'(z)}{Q(z)}] \approx \frac{1}{m} \sum_{j=1}^m \exp(z_j)\frac{n^{-1}}{Q(z_j)} \implies \sum_j Z \approx \frac{1}{m} \sum_{k=1}^m \frac{\exp(z_j)}{Q(z_j)}
$$
