...
- **Everything that lands on GitHub is written in English** — PR titles and descriptions, commit messages, issue and review comments — even when the working conversation is not. The only things quoted verbatim are the artifacts' own names: a post's title, its genre tag, or a line of its text.
- Post content itself is written in whatever language the post is in. The English rule covers the repo's metadata, not the writing.
- PRs are **squash-merged only** (merge commits and rebase merges are disabled) and head branches auto-delete on merge. The squash commit takes its message from the branch commits, not the PR body.
